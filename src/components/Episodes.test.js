...
test('Renders episodes when data fetch finishes', () => {
    // mimic component when the app first renders
    const { rerender } = render(<Episodes episodes={[]} />)

    //practice with negative assertions
    let epiArr = screen.queryAllByTestId(/episodes/i)
    expect(epiArr).toHaveLength(0)

    rerender(<Episodes episodes={episodeData} />)

    epiArr = screen.getAllByTestId(/episodes/i)
    console.log(epiArr[0])
    expect(epiArr).toHaveLength(8)
})